...
**Data:** Same as Stage A, plus improving equity **EOD** coverage (Phase II row `equity_daily_ohlcv`).
...
**Data:** **Alt data** and **NLP** become industry-wide post-2010 (Phase II rows `news_wire_archives`, `sentiment_scores`, `satellite_imagery` with later `earliest_year`).
...
Discrete stages help readers; the evidence supports **overlapping** evolution more than sharp boundaries [[claim:CLM-2024-002]]. Phase III will formalize **signal hypotheses** (SIG-001 onward) rather than repeating performance folklore here.
